...
- `node-renderer` runs the React on Rails Pro boot seed before `/usr/local/node/bin/node renderer/node-renderer.js` so the new renderer cache is warm before Rails rolls.
