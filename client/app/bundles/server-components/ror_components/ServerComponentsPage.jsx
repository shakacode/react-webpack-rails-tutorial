...
function ServerComponentsPage({ comments = [] }) {
  return (
    <div className="max-w-4xl mx-auto py-8 px-4">
      <header className="mb-10">
        <h1 className="text-3xl font-bold text-slate-800 mb-2">React Server Components Demo</h1>
        <p className="text-slate-600 text-lg">
          This page is rendered using <strong>React Server Components</strong> with React on Rails Pro. Server
          components run on the server and stream their output to the client, keeping heavy dependencies out
      ...
      <div className="space-y-8">
        ...
        <section>
          ...
          <TogglePanel title="How does this work?">
            <div className="prose prose-slate max-w-none text-sm">
              <p>
                This toggle is a <code>&apos;use client&apos;</code> component, meaning it ships JavaScript to
                the browser for interactivity. But the content inside is rendered on the server and passed as
                children — a key RSC pattern called the <strong>donut pattern</strong>.
              </p>
              <ul>
                <li>
                  The <code>TogglePanel</code> wrapper runs on the client (handles click events)
                </li>
        ...
        <section>
          ...
          <p className="text-slate-500 text-sm mb-4">
            Comments come from the Rails controller as props — the canonical React on Rails Pro pattern. The
            page shell renders immediately while this section streams in progressively as Suspense boundaries
            resolve.
          </p>
          <Suspense
            fallback={
        ...
        <section className="bg-slate-50 border border-slate-200 rounded-xl p-6">
          <h2 className="text-lg font-semibold text-slate-700 mb-3">What makes this different?</h2>
          <div className="grid md:grid-cols-2 gap-4 text-sm text-slate-600">
            ...
            <div>
              ...
              <p>
                Server components fetch data by calling your Rails API internally — no client-side fetch
                waterfalls or loading spinners for initial data.
              </p>
            </div>
            <div>
              <h3 className="font-medium text-slate-800 mb-1">Progressive Streaming</h3>
              <p>
                The page shell renders instantly. Async components (like the comments feed) stream in as their
                data resolves, with Suspense boundaries showing fallbacks.
              </p>
            </div>
            <div>
              <h3 className="font-medium text-slate-800 mb-1">Selective Hydration</h3>
              <p>
                Only client components (like the toggle above) receive JavaScript. Everything else is pure
                HTML — zero hydration cost.
              </p>
            </div>
          </div>
        </section>
